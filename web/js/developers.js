...
function renderDevelopers() {
  ...
  const developers = contentData[currentLanguage].developers.team;
  const roles = contentData[currentLanguage].developers.roles;

  developersGrid.innerHTML = '';

  developers.forEach((dev) => {
    const roleLabel = roles[dev.role] || dev.role;
    ...
    card.innerHTML = `
      ...
      <div class="developer-info">
        ...
        <p class="developer-role">${roleLabel}</p>
        <p class="developer-responsibility">${dev.responsibility}</p>
        <div class="developer-links">
          ${dev.github ? `<a href="${dev.github}" target="_blank" rel="noopener noreferrer" class="dev-link github-link" title="GitHub">
          ...
          ${dev.x ? `<a href="${dev.x}" target="_blank" rel="noopener noreferrer" class="dev-link twitter-link" title="X (Twitter)">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
